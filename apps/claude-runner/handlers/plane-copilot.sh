...
LOG_DIR="{{USER_HOME}}/claude-runner/logs/plane-copilot"
mkdir -p "$LOG_DIR"
...
# Copilot CLI has no PostToolUse hooks, so only session_start/summary/end
# frames are captured (no per-tool entries).
export CLAUDE_HANDLER="${CLAUDE_HANDLER:-plane-copilot}"
...
# Verify this issue is actually assigned to Copilot before doing anything
COPILOT_USER_ID="${PLANE_COPILOT_USER_ID:?PLANE_COPILOT_USER_ID required}"
...
if ! echo "$ASSIGNEES" | grep -q "$COPILOT_USER_ID"; then
    echo "[$(date -u)] Issue not assigned to Copilot ($COPILOT_USER_ID), skipping."
    exit 0
fi
...
u = User.objects.get(email='copilot@{{DOMAIN}}')
...
# Build the prompt
PROMPT="You are GitHub Copilot running as an autonomous agent on a home server (Ubuntu 24.04).
...
echo "[$(date -u)] Copilot exited with code: $EXIT_CODE"
...
SUMMARY=$(echo "$COPILOT_OUTPUT" | tail -c 2000)

# Post comment and update state
if [ $EXIT_CODE -eq 0 ]; then
    COMMENT_BODY="<p>✅ <strong>GitHub Copilot completed {{PLANE_PROJECT_PREFIX}}-${ISSUE_SEQUENCE_ID}</strong></p><p><strong>Summary:</strong></p><p>$(echo "$SUMMARY" | sed 's/&/\&amp;/g;s/</\&lt;/g;s/>/\&gt;/g' | sed 's/$/\\n/' | tr -d '\n' | sed 's/\\n/<br>/g')</p><p><em>Log: ${LOG_FILE}</em></p>"
    ...
    ERROR_COMMENT="<p>❌ <strong>GitHub Copilot failed on {{PLANE_PROJECT_PREFIX}}-${ISSUE_SEQUENCE_ID}</strong></p><p>Exit code: ${EXIT_CODE}</p><p>Check log: <code>${LOG_FILE}</code></p><p><pre>$(echo "$COPILOT_OUTPUT" | tail -c 500 | sed 's/&/\&amp;/g;s/</\&lt;/g;s/>/\&gt;/g')</pre></p>"
